...
AlephTopologySloop::AlephTopologySloop(ITraceMng* tm,
                                       AlephKernel *kernel,
                                       Integer index,
                                       Integer nb_row_size):
...
AlephTopologySloop::~AlephTopologySloop(){
...
// * AlephVectorSloop
// ****************************************************************************
AlephVectorSloop::AlephVectorSloop(ITraceMng* tm,
                                   AlephKernel *kernel,
...
// * ~AlephVectorSloop
// ****************************************************************************
AlephVectorSloop::~AlephVectorSloop(){
 ...
 * AlephVectorCreate
 *****************************************************************************/
void AlephVectorSloop::AlephVectorCreate(void){
  ...
  if (!m_sloop_vector) throw Exception("AlephSolverVector::create()", " new SLOOPDistVector failed");
...
// * AlephVectorSet
// ****************************************************************************
void AlephVectorSloop::AlephVectorSet(const double *bfr_val, const int *bfr_idx, Integer size){
  debug()<<"\t[AlephVectorSloop::AlephVectorSet]";
  if (m_sloop_vector->locfill(bfr_val, bfr_idx, size))
	 throw Exception("AlephVectorSloop::AlephVectorSet", "locfill() failed");
}

/******************************************************************************
 * AlephVectorAssemble
 *****************************************************************************/
int AlephVectorSloop::AlephVectorAssemble(void){
  ItacFunction(AlephVectorSloop);
 ...
 * AlephVectorGet
 *****************************************************************************/
void AlephVectorSloop::AlephVectorGet(double *bfr_val, const int *bfr_idx, Integer size){
 ...
 * writeToFile
 *****************************************************************************/
void AlephVectorSloop::writeToFile(const String file_name){
  ItacFunction(AlephVectorSloop);
 ...
 * AlephMatrixSloop
 *****************************************************************************/
AlephMatrixSloop::AlephMatrixSloop(ITraceMng* tm,
                                   AlephKernel *kernel,
...
// * AlephMatrixSloop
// ****************************************************************************
AlephMatrixSloop::~AlephMatrixSloop(){
 ...
 * AlephMatrixCreate
 *****************************************************************************/
void AlephMatrixSloop::AlephMatrixCreate(void){
 ...
 *****************************************************************************/
void AlephMatrixSloop::AlephMatrixSetFilled(bool toggle){
  ItacFunction(AlephMatrixSloop);
 ...
 * AlephMatrixAssemble
 *****************************************************************************/
int AlephMatrixSloop::AlephMatrixAssemble(void){
  ItacFunction(AlephMatrixSloop);
 ...
 * AlephMatrixFill
 *****************************************************************************/
void AlephMatrixSloop::AlephMatrixFill(int size, int *rows, int *cols, double *values){
  ItacFunction(AlephMatrixSloop);
 ...
 * isAlreadySolved
 *****************************************************************************/
bool AlephMatrixSloop::isAlreadySolved(SLOOP::SLOOPDistVector* x,
...
/******************************************************************************
 *****************************************************************************/
int AlephMatrixSloop::AlephMatrixSolve(AlephVector* x,
                                       AlephVector* b,
                                       AlephVector* tmp,
  ...
  SLOOP::SLOOPDistVector* solution = (dynamic_cast<AlephVectorSloop*> (x->implementation()))->m_sloop_vector;
  SLOOP::SLOOPDistVector* RHS      = (dynamic_cast<AlephVectorSloop*> (b->implementation()))->m_sloop_vector;
  SLOOP::SLOOPDistVector* temp     = (dynamic_cast<AlephVectorSloop*> (tmp->implementation()))->m_sloop_vector;
  ...
  SLOOP::SLOOPStopCriteria *sc=createSloopStopCriteria(solver_param, *sloopParallelInfo->m_sloop_msg);
    
  SLOOP::SLOOPSolver *global_solver=createSloopSolver(solver_param, *sloopParallelInfo->m_sloop_msg);
    
  SLOOP::SLOOPPreconditioner *precond=createSloopPreconditionner(solver_param, *sloopParallelInfo->m_sloop_msg);
    
  this->setSloopSolverParameters(solver_param, global_solver);
  this->setSloopPreconditionnerParameters(solver_param, precond);
    
  SLOOP::SLOOPDistVector *diag;
 ...
 * writeToFile
 *****************************************************************************/
void AlephMatrixSloop::writeToFile(const String file_name){
  ItacFunction(AlephMatrixSloop);
 ...
 * createSloopSolver
 *****************************************************************************/
SLOOP::SLOOPSolver* AlephMatrixSloop::createSloopSolver(AlephParams* solver_param, SLOOP::SLOOPMsg& info_sloop_msg){
 ...
 *****************************************************************************/
SLOOP::SLOOPPreconditioner* AlephMatrixSloop::createSloopPreconditionner(AlephParams* solver_param,
 ...
 *****************************************************************************/
SLOOP::SLOOPStopCriteria * AlephMatrixSloop::createSloopStopCriteria(AlephParams* solver_param,
                                                                     SLOOP::SLOOPMsg& info_sloop_msg) {
  ItacFunction(AlephMatrixSloop);
...
/******************************************************************************
 *****************************************************************************/
void AlephMatrixSloop::setSloopSolverParameters(AlephParams* solver_param,
...
/******************************************************************************
 *****************************************************************************/
void AlephMatrixSloop::setSloopPreconditionnerParameters(AlephParams* solver_param,
...
/******************************************************************************
 *****************************************************************************/
bool AlephMatrixSloop::normalizeSolverMatrix(AlephParams* solver_param){
  ItacFunction(AlephMatrixSloop);
  switch (solver_param->precond()){
  case TypesSolver::AINV:
    //case TypesSolver::AMG:
...
SloopAlephFactoryImpl::SloopAlephFactoryImpl() :
  m_IAlephVectors(0),
  m_IAlephMatrixs(0),
  m_IAlephTopologys(0){}
SloopAlephFactoryImpl::~SloopAlephFactoryImpl(){
...
void SloopAlephFactoryImpl::initialize() {}
  
IAlephTopology* SloopAlephFactoryImpl::createTopology(ITraceMng* tm,
                                                      AlephKernel* kernel,
                                                      Integer index,
                                                      Integer nb_row_size){
...
IAlephVector* SloopAlephFactoryImpl::createVector(ITraceMng* tm,
                                                  AlephKernel* kernel,
                                                  Integer index){
...
IAlephMatrix* SloopAlephFactoryImpl::createMatrix(ITraceMng* tm,
                                                  AlephKernel* kernel,
                                                  Integer index){
